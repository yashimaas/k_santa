...
void init_data() {
    ifstream in("../input/family_data.csv");
...
int main() {
    init_data();
    auto assigned_day = read_submission("../input/submission_mcf.csv");
...
//    auto count_exit = [start = 0]() mutable { return (++start <= 1000); };
    auto time_exit = [start = high_resolution_clock::now()]() {
        return duration_cast<minutes>(high_resolution_clock::now()-start).count() < 1; //5h55
    };
